...
class BraveSearchProvider {
  ...
  /**
   * Synchronous usability check: a literal config key, a launch-environment
   * key, or a credentials-store key (sampled at apply time). Returns false
   * when none of the three sources can supply a key, so provider selection
   * does not mistake an unconfigured provider for a usable one.
   */
  available() {
    const options = this.options();
    return (options.apiKey?.length ?? 0) > 0 || options.envKey.length > 0 || this.storeState?.current === true;
  }
...
export function apply(ctx, config) {
  let current = () => config;
  // Sample credentials-store state once at apply time so the synchronous
  // available() can tell store-configured keys apart from "no key anywhere".
  // Resolution per search still re-reads the store, so rotations apply live.
